 ...
 *
 * WhatsApp (Twilio MediaUrl) and Slack (file_share event) will each get
 * their own `baget-<channel>-attachments.ts` file that returns the same
 * InboundAttachment[] shape.
 */
...
export function parseTelegramAttachments(msg: TelegramMessage): ParsedAttachment | null {
  if (msg.photo && msg.photo.length > 0) {
    ...
    return {
      kind: 'photo',
      fileId: best.file_id,
      mimeType: 'image/jpeg', // Telegram always serves photos as JPEG
...
export async function downloadTelegramAttachment(
  ...
  // Step 1: getFile to resolve the file_path on Telegram's servers
  ...
  // Post-download size check (in case getFile didn't report size)
